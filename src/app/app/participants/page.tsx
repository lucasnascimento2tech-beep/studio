...
export default function ParticipantsPage() {
  ...
  if (loading) return <div className="p-8 text-center">Carregando participantes...</div>;
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[500px]">
                ...
                <div className="bg-slate-50 p-4 rounded-xl space-y-4">
                  ...
                  {isMandatory && (
                    <div className="space-y-3 pl-6 border-l-2 border-primary/20">
                      <Label className="text-[10px] font-bold uppercase text-slate-400">Essencial nos encontros:</Label>
                      {[
                        { id: 'meeting_1_parametrizacao', label: '1. Parametrização' },
                        { id: 'meeting_2_operacao', label: '2. Operação' },
                        { id: 'meeting_3_financeiro', label: '3. Financeiro' }
                      ].map(meeting => (
                        <div key={meeting.id} className="flex items-center space-x-2">
                          <Checkbox 
                            id={meeting.id}
                            checked={mandatoryMeetings.includes(meeting.id)}
                            onCheckedChange={(c) => {
                              if(c) setMandatoryMeetings([...mandatoryMeetings, meeting.id]);
                              else setMandatoryMeetings(mandatoryMeetings.filter(m => m !== meeting.id));
                            }}
                          />
                          <Label htmlFor={meeting.id} className="text-xs">{meeting.label}</Label>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <DialogFooter>
                <Button className="w-full h-12" onClick={handleCreateInvite} disabled={isSubmitting}>
            ...
            <Card key={member.id} className="border-none shadow-md hover:shadow-lg transition-shadow">
              <CardHeader className="flex flex-row items-start justify-between pb-2">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-400 hover:text-red-500" onClick={() => removeMember(member.id)}>
